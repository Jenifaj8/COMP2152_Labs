...
def binary_search_iterative(nums, target):
    ...
    # While left <= right:
    while left < = right:
        mid = (left + right) //2    # - Calculate mid = (left + right) // 2
        if nums[mid] == target:
            return mid              # - If nums[mid] == target, return mid
        elif target < nums[mid]:
            right = mid -1          # - If target < nums[mid], search left half: right = mid - 1
        else:
            left = mid + 1          # - If target > nums[mid], search right half: left = mid + 1
...
def binary_search_recursive(nums, target, left, right):
    ...
    if nums[mid] == target:
        return mid              # If nums[mid] == target, return mid
    if target < nums[mid]:      # If target < nums[mid], recurse on left half
        return binary_search_recursive(nums, target, left, mid -1)   # Hint: return binary_search_recursive(nums, target, left, mid - 1)
